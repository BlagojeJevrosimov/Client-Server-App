...
MESSAGE Dequeue(MESSAGE_QUEUE* queue)
{
	...
	if (queue->front == -1) {
		...
		strcpy(message.messageText, NULL);
		strcpy(message.topic, NULL);

	}
	else {
		printf("\nDequeue operation sucessful\n");
		strcpy(message.topic, (queue->messages[queue->front]).topic);
		strcpy(message.messageText, (queue->messages[queue->front]).messageText);
